...
| 要改变什么 | 层级 | 主要目录 |
| --- | --- | --- |
| 模型可见的数学语义或 tensor contract | L3 Rust Semantic | `src/ops/<operator>/` 的公开 API、`cuda-operator.md` |
| normalize、execution cache、session 或 graph 生命周期 | L2 Rust Execution | `src/ops/<operator>/`、`src/workspace.rs`、`src/graph.rs`、`src/ffi/abi/` |
| C ABI、recipe、选核、autotune、fallback、candidate 或 provider 适配 | L1 C++ Native operator | `native/include/`、`native/adapters/`、`native/framework/` |
| CUDA 实现、模板实例或 vendor patch | L0 Kernel | `native/kernels/`、`native/patches/`、`build.rs` |

```text
model → L3 Rust → L2 Rust → C ABI → L1 C++ → L0 CUDA
```

新增 L0 kernel 必须先接入 L1；L2 只通过 C ABI 调用 L1；模型只调用 L3。核心对象和 key 见 [`apxinf-cuda-new` 架构说明](../crates/apxinf-cuda-new/README.md)。
...
Required types：`Spec`、`Implementation`、`Execution` 和 `ImplementationRegistry`。使用持久化调优时，
再增加只含一个 exact key 的 `TuningKeys`。

| Required function | MUST |
| --- | --- |
| `registry(spec.semantic)` | 返回该 semantic 的不可变 candidate 集合 |
...
| L1 prepare 方案 | 实现 |
| --- | --- |
| 唯一实现 | 直接 validate + prepare |
| 稳定启发式 | 在 adapter 内按 Spec/Policy 选择；复杂时才增加 `selection.cpp` |
| 需要实测 | exact recipe lookup；miss 时调用 `framework::autotune` |

| MUST NOT |
| --- |
| 在 `native/framework` 添加 semantic-specific shape/dtype/registry/fallback |
| 增加 compatible/hint key 或运行时 selection-mode metadata/dispatcher |
| 把 pointer/stream 放入持久 key |
| 在 capture 中首次 prepare |
