...
// SM100 FP8 widened paired gate/up projection with a TMEM-to-register epilogue.
//
// The static weight is tiled as [gate256, up256].  A single explicit-2SM
// 128x512x128 SM100 GEMM therefore consumes each A K-stage once and computes
// adjacent gate/up accumulator regions. Each small epilogue subtile loads
// gate and up from TMEM, rounds both scaled values through FP16 RNE, applies
// the production GeGLU arithmetic, and writes compact E4M3 output.  There is
// no full-N FP16 temporary.
...
namespace apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail {

constexpr int APXINF_DUAL_GEGLU_WIDE_PAIR_IMPLEMENTED = 1;
constexpr int APXINF_DUAL_GEGLU_PAIRED_EPILOGUE_IMPLEMENTED = 1;

constexpr int kAcceptedM = 522;
constexpr int kGeneralizedM = 533;
constexpr bool valid_production_m(int m) {
  return m == kAcceptedM || m == kGeneralizedM;
}
...
#define APXINF_DUAL_GEGLU_GEMM_ALPHA 0.00027272797f
#endif
#ifndef APXINF_DUAL_GEGLU_OUTPUT_SCALE
#define APXINF_DUAL_GEGLU_OUTPUT_SCALE 0.11277901785714285f
#endif
...
struct GeGluOperation : cutlass::epilogue::fusion::FusionOperation {
  using ElementOutput = apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::ElementOutput;
  using ElementCompute = apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::ElementCompute;
  using ElementSource = apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::ElementSource;
...
template <>
struct FusionCallbacksTraits<apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::GeGluEVT> {
  using DispatchPolicy = void;
  using Callbacks = apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::GeGluEVT;
  using Operation = apxinf::cuda::cutlass_ops::fp8_dual_geglu_detail::GeGluOperation;
...
struct WidePairGemm {
  ...
  // SM100's physical FP8 2SM UMMA atom is limited to N<=256.  The ordinary
  ...
  using LegalAtomTile = Shape<Int<128>, Int<256>, Int<128>>;
   ...
   public:
    ...
    struct SharedStorage {
      alignas(128) cutlass::Array<
          cutlass::half_t, RendezvousElements * RendezvousStages> gate;
      alignas(128) cutlass::Array<
          cutlass::half_t, RendezvousElements * RendezvousStages> up;
    };
    ...
    static bool can_implement(ProblemShape const& problem_shape,
      ...
      auto [m, n, k, l] = shape;
      bool exact_shape =
          valid_production_m(m) && n == kFullN && k == kK && l == 1;
    ...
    CUTLASS_DEVICE auto operator()(
      ...
      Tensor rGate = make_tensor<cutlass::half_t>(shape(rAcc));
      Tensor rUp = make_tensor<cutlass::half_t>(shape(rAcc));
        ...
        for (int epi_m = 0; epi_m < NumEpiM; ++epi_m) {
          ...
          if (warp_idx == gate_owner) {
            ...
            for (int i = 0; i < size(rAcc); ++i) {
              rGate(i) = cutlass::half_t(rAcc(i) * params_.alpha);
            }
            copy(tiled_t2r, tUpTile, rAcc);
            CUTLASS_PRAGMA_UNROLL
            for (int i = 0; i < size(rAcc); ++i) {
              rUp(i) = cutlass::half_t(rAcc(i) * params_.alpha);
            }
            CUTLASS_PRAGMA_UNROLL
            for (int i = 0; i < size(rD); ++i) {
              ...
              float value =
                  production_gelu(gate) * up * params_.inverse_scale;
              value = fminf(448.0f, fmaxf(-448.0f, value));
              rD(i) = ElementD(value);
            }
...
// Production entry: exact M522/M533 language Gate/Up only. The input weight must
// already use the physical [gate256,up256] column order validated by the
// load-time oracle. This path allocates no workspace and launches one node.
int production_dual_geglu(
  ...
  if (activation == nullptr || interleaved_weight == nullptr || output == nullptr ||
      !valid_production_m(m) || n != kN || k != kK || full_n != kFullN ||
      full_n != 2 * n ||
      ...
      (reinterpret_cast<uintptr_t>(output) & 0x7) != 0) {
    return -6;
  }

  using Gemm = WidePairGemm<Shape<_128, _512, _128>, Shape<_2, _2, _1>>;
  ...
  typename Kernel::EpilogueArguments epilogue{
      alpha, 1.0f / output_scale, static_cast<ElementOutput*>(output), stride_d};
