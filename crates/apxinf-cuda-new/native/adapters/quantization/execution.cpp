...
std::unique_ptr<Execution> prepare(
  ...
  if (execution->resource_bytes > execution->resource_limit &&
    ...
    throw Failure(APXINF_STATUS_UNSUPPORTED,
                  "Quantization candidate exceeds the workspace limit");
  }
  return execution;
}
...
extern "C" apxinf_status_t apxinf_quantization_prepare(
    apxinf_runtime_t runtime, const apxinf_quantization_spec_t* spec,
    const apxinf_quantization_policy_t* policy,
    const apxinf_quantization_bindings_t* bindings,
    apxinf_quantization_execution_t* output) {
  return apxinf::quantization::abi_boundary([&] {
    ...
    if (cudaStreamIsCapturing(static_cast<cudaStream_t>(bindings->stream),
      ...
      throw Failure(APXINF_STATUS_INVALID_ARGUMENT,
                    "Quantization preparation is not allowed during capture");
    }

    std::unique_ptr<apxinf::quantization::Execution> execution;
    for (const auto& implementation :
      ...
      execution = apxinf::quantization::prepare(
    ...
    execution->summary = std::string(execution->implementation->name) +
                         ...
                         " source=direct";
    auto wrapper = std::make_unique<apxinf_quantization_execution>();
    wrapper->state = std::move(execution);
    *output = wrapper.release();
  });
}

extern "C" apxinf_status_t apxinf_quantization_enqueue(
    apxinf_quantization_execution_t execution) {
  return apxinf::quantization::abi_boundary([&] {
    if (execution == nullptr || execution->state == nullptr) {
      throw Failure(APXINF_STATUS_INVALID_ARGUMENT,
                    "null Quantization execution");
    }
    apxinf::quantization::check_cuda(cudaSetDevice(execution->state->device));
    apxinf::quantization::check_cuda(
        execution->state->implementation->enqueue(*execution->state));
  });
}

extern "C" void apxinf_quantization_destroy(
    apxinf_quantization_execution_t execution) {
  delete execution;
}

extern "C" const char* apxinf_quantization_summary(
    apxinf_quantization_execution_t execution) {
