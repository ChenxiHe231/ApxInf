...
static void verify_graph(State& candidate,const apxinf_gemm_bindings_t& b){
 ...
 status=cudaGraphLaunch(exec,stream);if(status==cudaSuccess)status=cudaStreamSynchronize(stream);
 ...
 for(const auto& impl:implementations){
  ...
  for(int config:configs){
   try{
    ...
    check_cuda(impl.launch(*candidate,b));auto actual=read(out.p,count,spec.output_dtype,(cudaStream_t)b.stream);
    double error=0,norm=0;bool valid=true;
