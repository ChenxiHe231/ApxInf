...
fn gemm_all_candidates_match_torch() {
    ...
    .unwrap();
    assert_all_applicable_candidates_checked(prepared.summary(), &["cutlass-nvfp4-block16"]);
    for configuration in 0..3 {
        assert!(
            prepared
                .summary()
                .contains(&format!("cutlass-nvfp4-block16#{configuration}=pass(")),
            "NVFP4 configuration {configuration} was not validated: {}",
            prepared.summary()
        );
    }
        ...
        for configuration in 0..3 {
            assert!(
                prepared
                    .summary()
                    .contains(&format!("cutlass-nvfp4-block16#{configuration}=pass(")),
                "{name} configuration {configuration} was not validated: {}",
                prepared.summary()
            );
        }
        println!("NVFP4 {name}: {}", prepared.summary());
    }
}
...
fn gemm_bias_gelu_all_candidates_match_torch() {
    ...
    .unwrap();
    assert_all_applicable_candidates_checked(prepared.summary(), &["cutlass-nvfp4-bias-gelu"]);
    for configuration in 0..3 {
        assert!(
            prepared
                .summary()
                .contains(&format!("cutlass-nvfp4-bias-gelu#{configuration}=pass(")),
            "NVFP4 bias+GELU configuration {configuration} was not validated: {}",
            prepared.summary()
        );
    }
}
...
fn gemm_geglu_all_candidates_match_torch() {
    ...
    .unwrap();
    assert_all_applicable_candidates_checked(prepared.summary(), &["cutlass-nvfp4-geglu"]);
    for configuration in 0..3 {
        assert!(
            prepared
                .summary()
                .contains(&format!("cutlass-nvfp4-geglu#{configuration}=pass(")),
            "NVFP4 GeGLU configuration {configuration} was not validated: {}",
            prepared.summary()
        );
    }
}
