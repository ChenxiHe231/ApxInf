...
use crate::ffi::abi::{quantization as abi, status};
...
impl ExecutionKey {
    fn new(ctx: &CudaContext, normalized: &Normalized) -> Self {
        Self {
            ...
            scales: normalized.bindings.scales as usize,
            stream: normalized.bindings.stream as usize,
            scale_bits: normalized.bindings.scale.to_bits(),
            workspace_limit: normalized.policy.workspace_limit as u64,
            allow_fallback: normalized.policy.allow_fallback,
...
impl Drop for Execution {
    fn drop(&mut self) {
        ...
        unsafe { abi::apxinf_quantization_destroy(self.raw) }
    }
}
...
pub(crate) fn prepare(ctx: &CudaContext, normalized: Normalized) -> Result<Rc<Execution>> {
    ...
    if !crate::workspace::may_prepare_native_resources() {
        return Err(Error::Other(
            "Quantization execution cache miss during capture; prepare the same bindings first"
                .into(),
        ));
    }
    ...
    unsafe {
        status::check(abi::apxinf_quantization_prepare(
            ctx.runtime(),
            &normalized.spec,
            &policy,
            &normalized.bindings,
    ...
    if raw.is_null() {
        return Err(Error::Other(
            "native Quantization prepare returned a null execution".into(),
        ));
    }
    #[cfg(test)]
    let summary = unsafe { CStr::from_ptr(abi::apxinf_quantization_summary(raw)) }
        .to_string_lossy()
        .into_owned();
...
pub(crate) fn execute(ctx: &CudaContext, normalized: Normalized) -> Result<()> {
    ...
    unsafe { status::check(abi::apxinf_quantization_enqueue(execution.raw))? };
