...
impl CudaBuffer {
    ...
    pub(crate) fn into_tensor(self, shape: Shape, dtype: DType) -> Tensor {
        ...
        // SAFETY: `self` owns this CUDA allocation and is retained by the
        // handle. `as_tensor` checks that shape and dtype exactly cover it.
        let handle = unsafe {
...
///
/// This is used for small graph inputs (for example token and position IDs)
/// which the host updates between graph replays. The allocation is portable,
/// but its device address is resolved for one concrete CUDA device.
...
// SAFETY: ownership can move between threads. The type is deliberately not
// Sync because `write_u32s(&self, ..)` mutates the mapped allocation.
unsafe impl Send for HostMappedBuffer {}

impl HostMappedBuffer {
    pub fn alloc(len: usize, device: usize) -> Result<Self, String> {
        if len == 0 {
            return Err("mapped CUDA allocation must be non-empty".into());
        }
        unsafe {
            ffi::check_cuda(ffi::cudaSetDevice(device as i32))?;
        }
        ...
        unsafe {
            ...
            if let Err(error) =
                ffi::check_cuda(ffi::cudaHostGetDevicePointer(&mut device_ptr, host_ptr, 0))
            {
                let _ = ffi::cudaFreeHost(host_ptr);
                return Err(error);
            }
    ...
    pub fn write_u32s(&self, values: &[u32]) -> Result<(), String> {
        ...
        if bytes > self.len {
            return Err(format!("mapped buffer is {} bytes, need {bytes}", self.len));
        }
        unsafe {
    ...
    pub fn address_at(&self, byte_offset: usize, len: usize) -> Result<CudaDeviceAddress, String> {
        ...
        Ok(CudaDeviceAddress {
            ptr: unsafe { self.device_ptr.cast::<u8>().add(byte_offset).cast() },
            len,
            device: self.device,
        })
    }
...
impl Drop for HostMappedBuffer {
    fn drop(&mut self) {
        ...
        unsafe {
            // cudaHostGetDevicePointer was resolved while this device was
            // current. Restore that association before releasing the mapping.
            let _ = ffi::cudaSetDevice(self.device as i32);
